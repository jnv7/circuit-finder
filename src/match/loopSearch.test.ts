...
import { LOOP_RESULT_COUNT, searchRoutedLoops, tryRouteLoop } from './loopSearch'
...
function disconnectedQuad(anchor: Point): Street[] {
  const [a, b, c, d] = corners(anchor)
  const gap = 8
...
describe('searchRoutedLoops', () => {
  ...
  it('falls back to Phase 6-only output when no candidate in the pool can form a loop', () => {
